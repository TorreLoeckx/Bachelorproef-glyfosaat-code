...
# 2. SETUP OUTPUT
output_folder = 'boxplots_node1'
if not os.path.exists(output_folder):
    os.makedirs(output_folder)

# 3. SETTINGS
custom_order = ['bare', 'green', 'glyphosate', 'mechanical']
color_palette = {'green': 'green', 'bare': 'brown', 'mechanical': 'orange', 'glyphosate': 'red'}
...
for idx in indices:
    for stat in stats:
        ...
        if not cleaned_data_list: continue
        plot_df = pd.concat(cleaned_data_list)
        
        # --- DETAILED PLOTTING ---
        fig, ax = plt.subplots(figsize=(10, 8)) # Slightly taller for better detail visibility
        ...
        # This divides the space between major ticks into 10 intervals
        ...
        # Minor grid lines (the 9 lines in between)
