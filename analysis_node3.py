...
# 1. LOAD THE DATA
# Note: Ensure 'Indices_all_parcels.csv' is in your current working directory
...
# 5. NEW FILTERING & EXPORT
classifier_df = pd.DataFrame(final_data)

# Requirement 1: Filter out parcels that belong to 'green'
classifier_df = classifier_df[classifier_df['parcel_type'] != 'green']

# Requirement 2: Filter out parcels with a total slope NDVI higher than -0.00315
# This means keeping only parcels where 'mean_ndvi_total_slope' <= -0.00315
if 'mean_ndvi_total_slope' in classifier_df.columns:
    classifier_df = classifier_df[classifier_df['mean_ndvi_total_slope'] <= -0.00315]

# Final sorting and category ordering (excluding 'green' and 'bare')
custom_order = ['glyphosate', 'mechanical']
...
# Export to analysis_node3.csv
classifier_df.to_csv('analysis_node3.csv', index=False)
