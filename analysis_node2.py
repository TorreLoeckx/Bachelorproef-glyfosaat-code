...
# Requirement 1: Filter out parcels that belong to 'bare soil'
classifier_df = classifier_df[classifier_df['parcel_type'] != 'bare']

# Requirement 2: Filter out parcels with a maximum NDVI lower than 0.575
# The NDVI max column is generated as 'mean_ndvi_max' in the feature extraction step
if 'mean_ndvi_max' in classifier_df.columns:
    classifier_df = classifier_df[classifier_df['mean_ndvi_max'] >= 0.575]

# Final sorting and category ordering
custom_order = ['green', 'glyphosate', 'mechanical'] # 'bare' removed from order
...
# Export to analysis_node2.csv
classifier_df.to_csv('analysis_node2.csv', index=False)
